...
LUAROCKS_MK_DATA = 
...
ifeq ($($(VPFX)_WASM),1)
CONFIG_DEPS += $(addprefix $(BUILD_DIR)/, $(patsubst %.lua,%, $(TEST)))
CONFIG_DEPS += $(addprefix $(BUILD_DIR)/, $(patsubst test/spec/%.lua,test/spec-pre/%.lua, $(TEST)))
else
CONFIG_DEPS += $(addprefix $(BUILD_DIR)/, $(TEST))
endif
...
$(BUILD_DIR)/test/spec-pre/%: test/spec/%
	echo "Templating '$<' -> '$@'"
	$(TOKU_TEMPLATE) -f "$<" -o "$@"

$(BUILD_DIR)/test/spec/%: $(BUILD_DIR)/test/spec-pre/%.lua
	echo "Bundling '$<' -> '$@'"
	mkdir -p "$(dir $@)"
	$(CLIENT_VARS) BUILD_DIR="$(BUILD_DIR)" toku bundle -L \
		--env $(VPFX)_SANITIZE "$($(VPFX)_SANITIZE)" \
		--env LUACOV_CONFIG "$(TEST_LUACOV_CFG)" \
		--path "$(shell $(TEST_LUAROCKS) path --lr-path);?.lua" \
		--cpath "$(shell $(TEST_LUAROCKS) path --lr-cpath)" \
		--input "$<" --output-directory "$(dir $@)" --output-prefix "$(notdir $@)"
